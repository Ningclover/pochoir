...
def draw_strip(dom,holeWidth,betweenHoles,Nstrips,potential,cfg):
    ...
    if cfg==0:
        ...
        strip[dH+2*dB:3*dH+2*dB]=0
        strip[3*dH+2*dB:]=potential
    if cfg==1:
        strip[0:dB]=potential
        strip[dB:dB+2*dH]=0
        strip[dB+2*dH:3*dB+2*dH]=potential
        strip[3*dB+2*dH:]=0
    return strip
...
def generator(dom, cfg):
    ...
    if plane==1:
        draw_hole_pattern(arr,barr,dom,z_i2,widthX,widthZ,holeWidth,betweenHoles,Nstrips,1)
    if plane==2:
        draw_hole_pattern(arr,barr,dom,z_i1,widthX,widthZ,holeWidth,betweenHoles,Nstrips,1)
